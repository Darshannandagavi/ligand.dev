...
export const register = async (req, res) => {
  try {
    ...
    console.log("req.body is here",name, email, contact, password, usn, year, batch, collegeName, programName, technology);
...
export const makeBatchPassout = async (req, res) => {
    try {
        ...
        if (!collegeName || year === undefined || year === null || year === "") {
            return res.status(400).json({ error: "collegeName and year are required" });
        }
    ...
    return res.json({ modifiedCount: result.modifiedCount ?? 0, message: `${result.modifiedCount ?? 0} users marked as passout` });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
